...
(defbitfield (SWP_FLAG :uint)
  (:SWP_ASYNCWINDOWPOS #x4000) ;If the calling thread and the thread that owns the window are attached to different input queues, the system posts the request to the thread that owns the window. This prevents the calling thread from blocking its execution while other threads process the request.
  (:SWP_DEFERERASE #x2000);Prevents generation of the WM_SYNCPAINT message.
  (:SWP_DRAWFRAME #x0020);Draws a frame (defined in the window's class description) around the window.
  (:SWP_FRAMECHANGED #x0020);Applies new frame styles set using the SetWindowLong function. Sends a WM_NCCALCSIZE message to the window, even if the window's size is not being changed. If this flag is not specified, WM_NCCALCSIZE is sent only when the window's size is being changed.
  (:SWP_HIDEWINDOW #x0080);Hides the window.
  (:SWP_NOACTIVATE #x0010);Does not activate the window. If this flag is not set, the window is activated and moved to the top of either the topmost or non-topmost group (depending on the setting of the hWndInsertAfter parameter).
  (:SWP_NOCOPYBITS #x0100);Discards the entire contents of the client area. If this flag is not specified, the valid contents of the client area are saved and copied back into the client area after the window is sized or repositioned.
  (:SWP_NOMOVE #x0002);Retains the current position (ignores X and Y parameters).
  (:SWP_NOOWNERZORDER #x0200);Does not change the owner window's position in the Z order.
  (:SWP_NOREDRAW #x0008);Does not redraw changes. If this flag is set, no repainting of any kind occurs. This applies to the client area, the nonclient area (including the title bar and scroll bars), and any part of the parent window uncovered as a result of the window being moved. When this flag is set, the application must explicitly invalidate or redraw any parts of the window and parent window that need redrawing.
  (:SWP_NOREPOSITION #x0200);Same as the SWP_NOOWNERZORDER flag.
  (:SWP_NOSENDCHANGING #x0400);Prevents the window from receiving the WM_WINDOWPOSCHANGING message.
...
(defparameter +PS_STYLE_MASK+  #x0000000F)
(defparameter +PS_ENDCAP_MASK+ #x00000F00)
(defparameter +PS_JOIN_MASK+   #x0000F000)
(defparameter +PS_TYPE_MASK+   #x000F0000)

(defcenum (PS_STYLE_ENUM :uint)
  (:PS_SOLID            0)
  (:PS_DASH             1);       /* -------  */
  (:PS_DOT              2);       /* .......  */
...
(defcenum (PS_ENDCAP_ENUM DWORD)
  (:PS_ENDCAP_ROUND     #x00000000)
  (:PS_ENDCAP_SQUARE    #x00000100)
  (:PS_ENDCAP_FLAT      #x00000200))

(defcenum (PS_JOIN_ENUM DWORD)
  (:PS_JOIN_ROUND       #x00000000)
  (:PS_JOIN_BEVEL       #x00001000)
  (:PS_JOIN_MITER       #x00002000))

(defcenum (PS_TYPE_ENUM DWORD)
  (:PS_COSMETIC         #x00000000)
  (:PS_GEOMETRIC        #x00010000))

(defcenum (BRSH_ENUM :uint)
  (:BS_SOLID            0)
  (:BS_NULL             1)
  (:BS_HOLLOW           1)
  (:BS_HATCHED          2)
...
(defcstruct LOGPEN
  (:lopnStyle PS_STYLE_ENUM)
  (:lopnWidth (:struct POINT))
  (:lopnColor COLORREF))

(defcstruct LOGBRUSH
  (:lbStyle BRSH_ENUM)
  (:lbColor COLORREF)
  (:lbHatch ULONG_PTR));HS_ENUM or pointer to bitmap

(defcstruct EXTLOGPEN
  (:elpPenStyle DWORD) ;PS_ENUM, PS_FLAG
  (:elpWidth DWORD)
  (:elpBrushStyle BRSH_ENUM)
  (:elpColor  COLORREF)
  (:elpHatch ULONG_PTR);HS_ENUM or pointer to bitmap
  (:elpNumEntries DWORD)
  (:elpStyleEntry DWORD));undefinite length

(defcenum (STKOBJ_ENUM :int)
  (:WHITE_BRUSH         0)
  (:LTGRAY_BRUSH        1)
  (:GRAY_BRUSH          2)
  (:DKGRAY_BRUSH        3)
...
(defcenum (IMAGE_TYPE_ENUM :uint)
  (:IMAGE_BITMAP 0)			;Loads a bitmap.
  (:IMAGE_CURSOR 2)			;Loads a cursor.
  (:IMAGE_ICON 1))				;Loads an icon.

(defbitfield (IMAGE_LR_FLAG :uint)
    (:LR_CREATEDIBSECTION #x00002000) ;When the uType parameter specifies IMAGE_BITMAP, causes the function to return a DIB section bitmap rather than a compatible bitmap. This flag is useful for loading a bitmap without mapping it to the colors of the display device.
  (:LR_DEFAULTCOLOR #x00000000) ;The default flag; it does nothing. All it means is "not LR_MONOCHROME".
  (:LR_DEFAULTSIZE #x00000040) ;Uses the width or height specified by the system metric values for cursors or icons, if the cxDesired or cyDesired values are set to zero. If this flag is not specified and cxDesired and cyDesired are set to zero, the function uses the actual resource size. If the resource contains multiple images, the function uses the size of the first image.
  (:LR_LOADFROMFILE #x00000010) ;Loads the stand-alone image from the file specified by lpszName (icon, cursor, or bitmap file).
  (:LR_LOADMAP3DCOLORS #x00001000) ;Searches the color table for the image and replaces the following shades of gray with the corresponding 3-D color.
  ...
  (:LR_LOADTRANSPARENT #x00000020) ;Retrieves the color value of the first pixel in the image and replaces the corresponding entry in the color table with the default window color (COLOR_WINDOW). All pixels in the image that use that entry become the default window color. This value applies only to images that have corresponding color tables.
					;Do not use this option if you are loading a bitmap with a color depth greater than 8bpp.
  ...
  (:LR_MONOCHROME #x00000001) ;Loads the image in black and white.
  (:LR_SHARED #x00008000) ;Shares the image handle if the image is loaded multiple times. If LR_SHARED is not set, a second call to LoadImage for the same resource will load the image again and return a different handle.
  ...
  (:LR_VGACOLOR #x00000080) ;Uses true VGA colors.
  )

(defcenum (BLEND_OP_ENUM C_BYTE)
  (:AC_SRC_OVER #x00))

(defbitfield (ALPHA_FORMAT_FLAG C_BYTE)
  (:AC_SRC_OPAQUE #x00)
  (:AC_SRC_ALPHA #x01))

(defcstruct BLENDFUNCTION 		;hack: tricks! it is just a DWORD?; cffi cant support passing struct 
  (:BlendOp BLEND_OP_ENUM)
  (:BlendFlags C_BYTE)
  (:SourceConstantAlpha C_BYTE)
  (:AlphaFormat ALPHA_FORMAT_FLAG))
